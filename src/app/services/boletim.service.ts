import { Boletim, Disciplina } from './../models/boletim.model';
...
export class BoletimService {
  ...
  loadDadosBoletim(idGoogle: string): Observable<Boletim[]>{
    let colecao: string = `escolas/${this.principal.escola}/anosLetivos/${this.principal.anoLetivo}/boletins`
    return this.firestore.collection<Boletim>(colecao, ref=> ref.where('idGoogle','==', idGoogle).orderBy('nomeDisciplina'))
      .valueChanges()
  }
}
